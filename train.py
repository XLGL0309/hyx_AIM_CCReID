...
def train_aim(config, epoch, model, model2, classifier, clothes_classifier, clothes_classifier2, fuse, criterion_cla,
              criterion_pair,
    ...
    for batch_idx, (imgs, pids, camids, clothes_ids, img_path) in enumerate(trainloader):
        ...
        corrects2.update(torch.sum(pred_clothes2 == clothes_ids.data).float() / clothes_ids.size(0),
                         clothes_ids.size(0))
        ...
        clothes_corrects.update(torch.sum(clothes_preds == clothes_ids.data).float() / clothes_ids.size(0),
                                clothes_ids.size(0))
        clothes_corrects2.update(torch.sum(pred_clothes2 == clothes_ids.data).float() / clothes_ids.size(0),
                                 clothes_ids.size(0))
        batch_cla_loss.update(cla_loss.item(), pids.size(0))
        batch_pair_loss.update(pair_loss.item(), pids.size(0))
        batch_clo_loss.update(clothes_loss.item(), clothes_ids.size(0))
        batch_adv_loss.update(adv_loss.item(), pids.size(0))
