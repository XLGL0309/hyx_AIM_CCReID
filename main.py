...
def parse_option():
    ...
    parser.add_argument('--dataset', type=str, default=None, help="ltcc, prcc, vcclothes, last, deepchange")
    # Miscs
...
def main(config):
    ...
    # ==========  核心修改：将pid2clothes移动到指定设备，解决跨设备索引问题 ==========
    pid2clothes = pid2clothes.to(device)

    # Build model
    model, model2, fuse, classifier, clothes_classifier, clothes_classifier2 = build_model(config,
                                                                                           dataset.num_train_pids,
                                                                                           dataset.num_train_clothes)
    print("model loaded")
    # Build identity classification loss, pairwise loss, clothes classificaiton loss, and adversarial loss.
    criterion_cla, criterion_pair, criterion_clothes, criterion_adv, kl = build_losses(config,
                                                                                       dataset.num_train_clothes)
    print("loss built")
    # Build optimizer
    ...
    if config.MODEL.RESUME:
        # ==========  修改点2：添加map_location，适配单卡加载权重 ==========
        logger.info("Loading checkpoint from '{}'".format(config.MODEL.RESUME))
        checkpoint = torch.load(config.MODEL.RESUME, map_location=f'cuda:{config.GPU}')
    ...
    # ==========  修改点3：注释分布式local_rank相关代码，替换为单卡设备配置 ==========
    local_rank = 0  # 单卡环境固定为0
    # 复用提前定义的device，无需重新定义，消除冗余
    ...
    # ==========  修改点4：注释分布式数据并行（DDP）包装代码，保留原模型单卡运行 ==========
    """
    分布式DDP包装代码（单卡训练注释掉，如需恢复分布式请取消注释）：
    ...
    for epoch in range(start_epoch, config.TRAIN.MAX_EPOCH):
        ...
        """
        分布式sampler代码（单卡训练注释掉，如需恢复分布式请取消注释）：
        train_sampler.set_epoch(epoch)
        """
        start_train_time = time.time()

        train_aim(config, epoch, model, model2, classifier, clothes_classifier, clothes_classifier2, fuse,
        ...
        if (epoch + 1) > config.TEST.START_EVAL and config.TEST.EVAL_STEP > 0 and \
            ...
            # ==========  修改点8：添加model.eval()和with torch.no_grad()，单卡评估时节省显存 ==========
            model.eval()
            with torch.no_grad():
                if config.DATA.DATASET == 'prcc':
...
if __name__ == '__main__':
    ...
    # ==========  修改点11：注释分布式初始化代码，单卡训练无需NCCL ==========
    """
    分布式初始化代码（单卡训练注释掉，如需恢复分布式请取消注释）：
    dist.init_process_group(backend="nccl", init_method='env://')
    local_rank = dist.get_rank()
    """
    local_rank = 0  # 单卡环境固定local_rank为0，兼容日志函数
    # Set random seed
    set_seed(config.SEED + local_rank)
    # get logger
    if not config.EVAL_MODE:
        # ==========  修改点12：修改日志文件名，标注单卡运行 ==========
        output_file = osp.join(config.OUTPUT, 'log_train_single_gpu.log')
    else:
        # ==========  修改点13：修改日志文件名，标注单卡运行 ==========
        output_file = osp.join(config.OUTPUT, 'log_test_single_gpu.log')

    # ==========  新增：创建输出文件夹（避免文件夹不存在导致日志保存失败） ==========
