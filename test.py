...
def concat_all_gather(tensors, num_total_examples):
    ...
    兼容单卡环境：单卡时直接返回输入数据，不进行多卡收集
    '''
    outputs = []
    world_size = get_world_size()  # 替换直接调用 dist.get_world_size()

    for tensor in tensors:
        if world_size == 1:
            # 单卡环境：无需收集，直接截断到目标长度并返回
            output = tensor[:num_total_examples]
...
def test(config, model, queryloader, galleryloader, dataset):
    ...
    since = time.time()
    model.eval()
    # ===================== 修改：兼容单卡/分布式，获取 local_rank =====================
    local_rank = get_local_rank()  # 替换直接调用 dist.get_rank()
    # ==============================================================================
